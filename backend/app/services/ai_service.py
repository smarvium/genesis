...
class AIService:
    ...
    async def _generate_with_gemini_2_flash(self, user_input: str) -> Blueprint:
        ...
        """Phase 3: Generate blueprint using Gemini 2.0 Flash as fallback"""
        
        # Simplified prompt for Gemini 2.0 Flash
        prompt = f"""
        You are GENESIS AI - Advanced business automation architect.
        
        FOUNDER'S VISION: "{user_input}"
        
        Create a complete business automation blueprint in JSON format:
        {{
            "interpretation": "Deep understanding of the business vision and its automation potential",
            "suggested_structure": {{
                "guild_name": "Memorable name for the automation guild",
                "guild_purpose": "Clear purpose statement",
                "agents": [
                    {{
                        "name": "Agent name",
                        "role": "Professional role", 
                        "description": "Agent capabilities and intelligence",
                        "tools_needed": ["real_api_1", "real_api_2", "real_api_3", "real_api_4"]
                    }}
                ],
                "workflows": [
                    {{
        ...
        Guidelines:
        - Create 2-4 agents with real API tools (Stripe, Slack, SendGrid, etc.)
        - Focus on practical business automation
        - Ensure workflows create measurable value
        """
        
        try:
            logger.info(f"🤖 Phase 3: Generating blueprint with Gemini 2.0 Flash for: {user_input[:50]}...")
            ...
            if not json_content:
                logger.warning("⚠️ Phase 3: Failed to extract JSON from Gemini 2.0 Flash, using fallback")
                return await self._generate_phase3_fallback(user_input)
            
            parsed_result = json.loads(json_content)
            
            if not self._validate_phase3_blueprint(parsed_result):
                logger.warning("⚠️ Phase 3: Invalid blueprint from Gemini 2.0 Flash, using fallback")
                return await self._generate_phase3_fallback(user_input)
            ...
            logger.info(f"✅ Phase 3: Blueprint generated with Gemini 2.0 Flash: {blueprint.id}")
            return blueprint
